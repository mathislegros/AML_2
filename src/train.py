"""
Simple BCE + Jaccard Training Script
Based on teammate's proven 0.55 score configuration:
- UnetPlusPlus + ResNet34
- 0.25 BCE + 0.75 Jaccard loss
- ALL data with expert weighting
- 5-fold CV on expert data for validation
"""
...
class AllDataDataset(Dataset):
    """Dataset using ALL data (expert + amateur) with optional expert weighting"""
...
class BCEJaccardLoss(nn.Module):
    """
    Simple BCE + Jaccard loss - PROVEN at 0.55 score
    0.25 * BCE + 0.75 * Jaccard
    """
    ...
    def forward(self, pred, target, sample_weights=None):
        ...
        # Apply sample weights if provided (for expert weighting)
        if sample_weights is not None:
            # Reshape weights for broadcasting: [B] -> [B, 1, 1, 1]
            weights = sample_weights.view(-1, 1, 1, 1)
            bce_loss = (bce_loss * weights).mean()
        else:
            bce_loss = bce_loss.mean()

        # Jaccard loss (already handles reduction)
...
def train_one_epoch(model, loader, criterion, optimizer, device, expert_weight=1.0, scaler=None):
    model.train()
    running_loss = 0.0
    ...
    for images, masks, is_expert in loader:
        ...
        # Create sample weights: expert samples get higher weight
...
def main(args):
    ...
    log_file = os.path.join("runs", f"simple_jaccard_{timestamp}.txt")
    sys.stdout = Logger(log_file)

    print("=" * 80)
    print("SIMPLE BCE + JACCARD TRAINING (Proven 0.55 config)")
    print("=" * 80)
    print(f"\nTimestamp: {timestamp}")
    print(f"Args: {args}")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(expert_data)):
        ...
        # Combine amateur + train expert for training
        ...
        train_dataset = AllDataDataset(train_videos, transform=train_transform, use_clahe=args.use_clahe)
        val_dataset = AllDataDataset(val_expert, transform=val_transform, use_clahe=args.use_clahe)
        ...
        # Simple BCE + Jaccard loss
        criterion = BCEJaccardLoss(bce_weight=0.25, jaccard_weight=0.75)
        ...
        for epoch in range(args.epochs):
            ...
            if val_iou > best_iou:
                best_iou = val_iou
                patience_counter = 0
                model_path = os.path.join("models", f"simple_jaccard_fold{fold}.pth")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--expert_weight', type=float, default=2.0,
                        help='Weight multiplier for expert samples (default: 2.0)')
