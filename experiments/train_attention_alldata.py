"""
Attention U-Net Training with ALL DATA
- EfficientNet-B4 encoder + SCSE decoder attention
- ALL data (expert + amateur) with expert weighting
- BCE + Jaccard loss (proven)
- Validates on expert data only (matches test distribution)
"""
...
class AllDataDataset(Dataset):
    """Dataset using ALL data (expert + amateur) with expert weighting"""
...
class ExpertOnlyDataset(Dataset):
    """Dataset for validation - expert only"""
...
def main(args):
    ...
    log_file = os.path.join("runs", f"attention_alldata_{timestamp}.txt")
    sys.stdout = Logger(log_file)

    print("=" * 80)
    print("ATTENTION U-NET + ALL DATA (Best of Both Worlds)")
    print("=" * 80)
    print(f"\nTimestamp: {timestamp}")
    print(f"Args: {args}")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(expert_data)):
        ...
        val_dataset = ExpertOnlyDataset(val_expert, transform=val_transform, use_clahe=args.use_clahe)
        ...
        for epoch in range(args.epochs):
            ...
            if val_iou > best_iou:
                best_iou = val_iou
                patience_counter = 0
                model_path = os.path.join("models", f"attention_alldata_fold{fold}.pth")
